...
Run `pnpm ui:dev` from the repository root for the independent fixture gallery at `http://127.0.0.1:4173`. It includes Side Panel and Control Center states without connecting to any Project or executor. Production keeps system fonts; only fixed visual tests load pinned local Inter/JetBrains Mono fonts. `pnpm ui:test` compares 24 stored light/dark/state screenshots and checks WCAG A/AA rules, keyboard controls, reduced motion and supported widths. `pnpm ui:update` explicitly replaces the baseline after visual review. Browser/platform identity is pinned in `test/visual/baseline/environment.json`; see [GUI acceptance](../../docs/GUI-ACCEPTANCE.md) for the matching browser and evidence.

Project/run lists window at most 14 rows and support Home/End/arrow navigation, including a 3,000-row fixture. The production-browser smoke measures 60 seconds of idle time against a real disposable local Daemon and checks zero API reads/UI mutations. Fixtures cannot complete real ChatGPT Pro acceptance.
