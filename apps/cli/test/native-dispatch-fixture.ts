...
export async function nativeDispatchFixture(
  ...
  const report = await withFixtureWorkspace(async ({ path }) => {
    ...
    assert.notEqual(initial.exitCode, 0, "Fixture must start with a failing test");
    ...
    try {
      ...
      const reader = `
        const {DaemonClient}=await import(${JSON.stringify(daemonUrl)});
        ...
        console.log(JSON.stringify({result,checks,view:await client.call('runs.get',locator)}));
      `;
      ...
      assert.equal(retrieved.view.status, "completed");
      ...
      assert.deepEqual(result.changedFiles, ["src/message.js"]);
      assert.deepEqual(
      ...
      for (const check of retrieved.checks) {
        ...
        assert.equal(check.event.success, true);
        assert.ok(check.event.results.length > 0);
        assert.ok(
          check.event.results.every((result: { exitCode: number }) => result.exitCode === 0),
        );
      }
      const patch = retrieved.checks.find((check: { id: string }) => check.id === "diff").event
        .results[0].stdout;
      assert.match(patch, /diff --git a\/src\/message\.js b\/src\/message\.js/);
