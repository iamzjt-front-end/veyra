...
it("wakes for new turns and an emptied composer, including input while a defer response is pending", async () => {
  vi.useFakeTimers();
  ...
  (article.firstElementChild as Element).textContent = frameHandoff(handoffTemplate(binding));
