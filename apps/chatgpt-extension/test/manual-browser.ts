...
document.querySelector('button').onclick=()=>{
 ...
 const match=[...text.matchAll(/\x60\x60\x60veyra-handoff\\n([\\s\\S]*?)\\n\x60\x60\x60/g)].at(-1);
 if(!match)return;
 ...
 setTimeout(()=>{code.textContent=JSON.stringify(handoff);stop.remove();const copy=document.createElement('button');copy.dataset.testid='copy-turn-action-button';article.append(copy);},100);
};
</script></body></html>`;
...
try {
  ...
  await popup.evaluate(() => document.querySelector<HTMLButtonElement>("#bind")?.click());
  ...
  assert.equal(
    messages.some((message) => message.includes(pairing.token)),
    false,
  );
  console.log(
    JSON.stringify({
