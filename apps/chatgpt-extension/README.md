...
`dist/build-info.json` 标识这次实际构建，包含输入指纹及各入口文件的 SHA-256。Diagnostics 显示扩展与页面接收器的构建标识；Bind/恢复握手要求两者一致。升级后的旧页面不会被当作新版本接收器。Chrome 的错误页可能保留旧记录：应比较错误 ID、次数和来源，并单独记录诊断探针的错误，不能用清空红标证明修复。

Diagnostics 的桥接确认记录按 Run 区分六个阶段：识别、校验、本机接收、执行结束、同一对话确认回传、审查保存。浏览器只保留最多 30 条阶段/时间/构建/标识元数据；交接单正文、执行结果和审查仍以 Project `.veyra/` 为准。未知接收器、未绑定对话、不可用 Project 或未确认原生 readiness 均不能显示 Ready。

派发前先保存交接单规范序列化的 SHA-256 指纹。响应丢失或重连时，只读查询同一 Project 的归档 handoff 和 Run；身份、指纹全部匹配才恢复观察原任务，不再次调用 dispatch。归档缺失、身份不符或查询失败时明确停在接收未确认阶段。手动 Pause/Unbind、原生授权变化和人工审批仍优先阻止恢复。

审查保存响应丢失时也只查询已有记录；未确认的写入不重发。已确认回传但尚未提交审查的绑定，恢复后只等待新的 assistant turn，不读取或重交历史审查。`FAIL + human` 表示验证失败并等待人工，映射为现有 Project 协议的 `fail + wait`，保存后停止自动桥接；不会擅自转成 repair。

页面恢复后，React 延迟挂载的旧消息同样被排除。只有当前页面新的真实发送动作，或 Veyra 即将回传本轮结果，才打开后续回答的接收边界；程序化 DOM 点击、历史消息重新挂载不会打开边界。人工审查停止后可显式点击「继续」：先只读核对同一 Run/Result 的已保存审查，再恢复等待新的任务，既不重派旧任务，也不批准任何原生执行审批。
