...
installContentOnce((restoreOnLoad) => {
  ...
  const bound = (id: string) =>
    !disposed && binding?.id === id && conversationUrl(location.href) === binding.conversation;
  function disarm() {
    binding = undefined;
    candidate = undefined;
    unwatch?.();
  ...
  const onMessage: Parameters<typeof chrome.runtime.onMessage.addListener>[0] = (
    ...
    if (disposed || sender.id !== runtime.id || !object(message)) return false;
  ...
  async function restore() {
    if (disposed || binding || busy || !conversationUrl(location.href)) return;
    try {
  ...
  return () => {
    ...
    try {
      ...
      // Reload can invalidate the old runtime; local observers/timers are already stopped.
    }
  };
});
