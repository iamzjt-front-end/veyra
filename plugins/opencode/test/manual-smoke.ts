...
async function main() {
  if (process.env.VEYRA_LIVE_SMOKE !== "1") {
    console.error(
      "Set VEYRA_LIVE_SMOKE=1 to run the OpenCode smoke test with the installed CLI's existing authentication.",
    );
    return 2;
  }
  ...
  const report = await withFixtureWorkspace(async ({ path }) => {
    ...
    const policy = {
      ...
      read: {
        "*": "deny",
        [join(path, "AGENTS.md")]: "allow",
        [join(path, "src/message.js")]: "allow",
      },
      edit: { "*": "deny", [join(path, "src/message.js")]: "allow" },
    };
    ...
    await writeFile(join(path, "opencode.json"), nativeConfig);
    const testPath = join(path, "test/message.test.js");
    const expected = "Hello from the verified OpenCode fixture";
    ...
    const result = await adapter.run(
      {
        runId: "opencode-smoke",
        stepId: "execute",
        role: "executor",
        goal: `Make message() return exactly "${expected}" by changing only src/message.js.`,
        instructions:
          "Read AGENTS.md, use native read/edit tools only, preserve tests and instructions. Do not run commands; Veyra runs tests afterward.",
    ...
    if (result.status !== "success")
      return {
        status: result.status === "needs_input" ? "blocked" : "failed",
        openCodeVersion: readiness.version,
        result,
      };
    assert.equal(await readFile(join(path, "AGENTS.md"), "utf8"), instructions);
    assert.equal(await readFile(join(path, "opencode.json"), "utf8"), nativeConfig);
