...
Optional `session: { project }` creates native-owned resumable work in an explicitly matching Project directory. `session: { project, resume: safeReference }` continues the exact native UUID for the same Veyra run. `AgentResult.session` returns only a validated safe locator; no credentials or native history are copied. Without session selection, execution remains ephemeral. See the repository's [Codex session guide](https://github.com/iamzjt-front-end/veyra/blob/main/docs/CODEX.md#explicit-project-session-continuity).
