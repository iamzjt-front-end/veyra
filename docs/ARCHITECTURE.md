...
Bridge reliability receipts remain surface-owned routing metadata: detected, validated, accepted, completed, delivered and reviewed, with bounded timestamps, build/Run IDs and references. A dispatch intent includes the SHA-256 of the canonical serialized handoff. Recovery requires the matching immutable archive and coordinator Run before observing accepted work; it never retries dispatch. Missing or conflicting evidence stays unresolved. Current-conversation Ready additionally requires a confirmed receiver running the worker's build, explicit binding, available Project and checked native readiness. Build identity and artifact digests are emitted in the installed extension's `build-info.json`; historical Chrome errors are distinguished by ID/count/source, not cleared as a repair mechanism.

Restored observation starts closed: delayed React hydration is historical even when a message ID was absent from the initial snapshot. A trusted current-page send event or the bridge's upcoming result delivery establishes the next-response boundary, excluding identities already present at that point. Scripted clicks and DOM layout mutations cannot open that boundary. No transcript is persisted or read to reconstruct missed work.
...
The observed compact `FAIL + human` combination maps to the existing `fail + wait` contract: a negative finding may require a human decision, not necessarily automatic repair. It is persisted as reported and stops bridging without satisfying approval gates. After a rejected review, a confirmed delivered result with no attempted review write can be restored to await a new turn. An uncertain submitted review must first be found in the archive; neither case replays historical page content.

An explicit local Resume can acknowledge a human-decision stop only after reading the matching immutable Run/Result review. This acknowledgement is surface routing metadata; it permits waiting for a new validated handoff, never dispatches the old task or satisfies native/Core approval gates. Old receipts recover their decision-needed flag from Project evidence, without guessing from display text.
