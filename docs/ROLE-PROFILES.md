...
Core looks up the resolved role, adds an independent profile snapshot as `AgentInput.profile`, and composes instructions in this order: general workflow-step guidance, role-profile instructions, literal step instructions, and any group-specific instructions. Group and step guidance refine the task within the declared role and existing project/provider constraints. The adapter's wire-output schema remains authoritative; the profile's `result` field describes the normalized result, not a replacement API response envelope.
