...
Core scheduling, named inputs, crash recovery and subscribers use these restored events. The CLI uses `eventView(event)` from `@veyra/core` for bounded JSON run/resume output and review inspection. Other surfaces can use the same helper. A preview is display text, not a complete event or a replacement for execution evidence. Large result references also appear in subsequent agents' bounded artifact context; Core does not follow those paths into prompts.
