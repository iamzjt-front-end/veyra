...
Every workspace already has a Vitest test script. `pnpm test` schedules one workspace task at a time with Turbo, retaining parallel suites and concurrency scenarios within each package. This avoids multiplying Vitest worker pools and their subprocesses across workspaces on small CI runners. The shared `vitest.config.ts` discovers `test/**/*.test.ts` relative to each invocation's working directory: package scripts find their owned tests, and the root invocation finds shared tests while excluding fixture projects. `tsconfig.test.json` checks tests without emitting files into package builds. Turbo invalidates cached tasks when shared helpers or fixtures change. Default tests do not generate coverage reports.

Core and CLI package test scripts limit Vitest to four workers because their integration suites exercise synced filesystem writes, process-death recovery and multiple native-client subprocesses. Project snapshot preservation tests compare complete file buffers with `Buffer.equals` so oversized fixtures do not incur deep object traversal. The real-Git workspace suites use a 30-second per-test bound to accommodate multiple subprocesses under concurrent load; the repeated child-scope consensus case allows 15 seconds for its two persisted invocations. These limits leave behavioral assertions intact; time-sensitive Runtime tests retain their explicit execution deadlines.
...
The same tarball test changes only extracted manifests to a fixture prerelease version and checks `ve version`, all eight built-in registrations and adapter descriptors in fresh processes. `test/versioning.test.ts` runs the pinned Changesets CLI against temporary copies of the workspace package graph. It checks patch/minor/major plans, generated changelogs, private-package exclusion, unknown-target rejection, repeated versioning and unchanged Git HEAD/tags. It also exercises `pnpm release:version` with offline dependency resolution and verifies the resulting frozen lockfile/formatting. Run `pnpm versioning:check` for just these tests; fixture cleanup runs even on failure. See [Versioning](VERSIONING.md).

`test/release.test.ts` checks release CI's standalone script against real Git/package fixtures and a fake npm executable. It tests versions/tags, ordered tarballs and hashes, dirty/pending-note rejection, publication approval checks, whole-bundle validation before registry operations, exact provenance/publication arguments, mismatched registry integrity and suppressed diagnostics. `pnpm release-ci:check` runs these cases; no test uses a real npm publisher. The script is type-checked through `tsconfig.scripts.json`. See [Release CI](RELEASING.md) for the separate hosted validation and human approval boundary.
