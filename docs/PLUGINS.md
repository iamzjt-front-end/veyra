...
Return the same scoped `AgentReadiness` contract as adapter discovery. CLI doctor includes scope, optional version and validated adapter descriptors when construction succeeds. OpenAI, Claude and Gemini hooks check credential-variable presence only; Codex and Claude Code use bounded native CLI support/authentication checks. Plugin loading and ordinary workflow execution do not automatically probe services.
