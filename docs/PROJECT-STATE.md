...
`createSession(reference)` / `getSession(runId)` use the same bounded immutable store for `<run-uuid>.session.json`. The safe reference contains only version, kind, provider, native UUID, Project ID, run UUID and creation time. Unknown fields, credentials, history, wrong Project/run links and unsafe native IDs are refused. It is an optional locator into native-owned state, never a prerequisite for recovering the Project's engineering context. A missing native session requires explicit reconciliation/new work; it does not authorize replay or scraping native storage. Execution results may carry this reference, and the daemon archives it alongside the handoff/result.
