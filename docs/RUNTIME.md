...
On Windows, Node's direct child termination is used. Reliable descendant cleanup needs Windows job-object/process-tree support, which is not provided by this implementation. The POSIX descendant and signal-escalation tests are explicitly platform-specific; the other process tests are portable. Node's [child-process documentation](https://nodejs.org/api/child_process.html#optionsdetached) describes the differing process-group semantics.
