...
On a fresh installation, missing pnpm produces setup guidance in plain text and `pnpm.message` in JSON. A missing default configuration reports `config.message` with `ve init` guidance. Doctor never creates these files or installs tools automatically.

Default `ve doctor` reports `mode: "native"`: Node/pnpm versions, platform, working-directory access and a required native Codex executor probe. The `executor` record distinguishes missing executable, installed but unauthenticated, inconclusive and ready states, includes a detected version and identifies the native client as login owner. `--codex-executable <path>` selects an existing binary for this probe. `OPENAI_API_KEY` absence is optional and cannot fail the native path; an API key cannot make a missing native executor ready. Unrelated native clients and third-party workflow plugins are not probed by default. Credentials and raw login output are never printed.

`ve doctor --config veyra.yaml` or `--workflow <name/path>` reports `mode: "workflow"` and explicitly checks an optional configured workflow. Referenced agents are required, other configured agents are optional, and an explicitly selected missing config fails. This mode uses the configured adapter executable options instead of `--codex-executable`. OpenAI, Claude and Gemini readiness check only environment-variable presence; they do not make an API call or prove account/model access. Native adapters use their own bounded support/auth checks. The existing [Claude](CLAUDE.md), [Claude Code](CLAUDE-CODE.md) and [Gemini](GEMINI.md) integrations remain available when selected deliberately.

For configured agents with valid metadata, `doctor --config veyra.yaml --json` also returns a `descriptor` with the adapter's version, model when configured, roles and advertised capabilities, plus the readiness `scope`. These are declarations of implemented adapter behavior, not live model capability tests. `workflow validate` checks the syntax of explicit `requires` constraints; Core matches descriptors before each invocation. See [capability discovery and requirements](CAPABILITIES.md).
...
For requested native verification, the local workflow supplies trusted command definitions and approval policy while the Project binding supplies the executor. No API agents are constructed. See [native dispatch and local checks](NATIVE-DISPATCH.md) for configuration and the opt-in live acceptance command.
