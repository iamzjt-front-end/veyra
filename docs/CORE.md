...
For the CLI's interrupted-run command, `recoverInterrupted: true` is an explicit assertion that the prior owner stopped. Core accepts only a proven completed step checkpoint with a valid successor, or a run-start boundary before execution. It records recovery and continues the successor without replaying completed mutations. Unknown in-flight agent/command completion remains refused. This narrow primitive supports CLI recovery; the workspace lease verifies an absent prior PID for that run, while general run locking and partial-transition reconciliation remain separate hardening work.

State and event writes are individually durable, not a single transaction across a provider's filesystem edits. Automatic recovery of arbitrary interrupted attempts and general state-store locking remain later hardening tasks. Do not treat a saved `running` state as permission to replay an interrupted mutating step blindly.
