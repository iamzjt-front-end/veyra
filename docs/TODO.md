...
- [-] Repair the reported no-dispatch/false-Ready state: fixed reproduced historical streaming flags, trusted composer-form submission and same-document reconnect gaps; added bounded bilingual page-stage diagnostics and selected-native-task readiness checks. Five baseline checks passed (2,249 tests), extension tests passed (214), plus existing-task Native Messaging/HTTP/startup/panel browser checks. Each transport completed two executions/two returns with persisted reviews despite stale streaming styles; both 3,000-turn/60-second idle checks made zero DOM queries. Actual archive inspection confirms the reported research task never reached dispatch; a fresh exact-native-task check still finds the desktop's active writer. The exact live DOM cause and actual shared-desktop activation remain unverified. See [handoff observation repair](HANDOFF-OBSERVATION-REPAIR.md). Never replay the old research task or ask the user to treat fixture success as a real fix.

- [-] Shared native service correction: opt-in private Unix WebSocket transport implemented so Veyra and the desktop can share one native writer, with idle/root checks, owned-turn cancellation, no automatic replay and persistent local transport selection. Real two-client adapter continuation, cancellation and NativeService → coordinator → Verifier evidence passed on an isolated durable task. Five repository checks passed (2,234 tests); extension tests (199), loopback and Native Messaging existing-task browser smoke passed, including reload, idle recovery and zero idle DOM queries. Actual desktop migration remains pending; its launch switch is experimental and concurrent desktop sends are not atomically excluded. See [shared-service evidence](CODEX-CONVERSATION-BINDING.md#shared-local-native-service--experimental-september-12).
...
- [!] Actual user task binding: the selected task **确认旧代码已删除** is discoverable at the expected `etf-quant-monitor` root, but the installed Codex desktop retains its exclusive writer. A metadata-only exact-ID `thread/resume` probe returned `already has an active writer`; no turn was submitted and no user source was changed. The experimental shared transport has native proof; the user's desktop still needs an approved restart/migration to that same service and actual binding verification. No takeover, fork, private desktop IPC or replacement task; see [evidence and support boundary](CODEX-CONVERSATION-BINDING.md).
...
### Live independent verification after failure — user report, 2026-09-10

**Status:** [x] Local repair and actual native success/failure evidence verified; real Pro re-acceptance remains [!].

The new real run completes native Codex in about 68 seconds, but its structured `failure` has no continuation into the independent Verifier. The generated command sequence also stops at the first failed check. The Project intentionally contains a failing greeting test; its configured build commands exist in `checks.yaml`, even though the executor guessed an absent npm build script. Keep executor claims distinct from actual Verifier evidence.

Collect all requested, locally configured checks after a settled executor result and ordinary check failures, retain negative execution/check outcomes, and preserve cancellation, timeouts, human gates and explicit stop policies. Supply the exact configured commands as executor guidance. Reproduce the real failure using the production composition, test success/failure and security boundaries, and verify actual native execution plus persisted evidence in disposable Projects before requesting another user test. Do not replay or modify the user's failed Project run or start P0.13–P0.15.

Verification:

- The real failed run `9df41990-6c71-4dd9-abf6-00c368e831ac` contains native `turn.completed`, process exit 0, executor `failure`, then `unhandled_step_failure`. The executor ran `npm test`, guessed the absent npm build script and collected a Git diff; those process records are executor evidence, not Verifier events. The authoritative build is `node --check src/message.js` plus `node scripts/build.mjs` in the local checks workflow.
- The generated sequence explicitly continues after ordinary failures and invokes each requested check at most once. Core's existing recovery-entry budget is allowed for first entry into an evidence step only; no graph edge returns to the agent or any prior check. Local approval, deadline and stop policies remain enforced. The last executor's negative result remains failed even if every independent check passes. Exact host-configured commands are supplied as guidance; model commands never become Verifier commands.
- Twelve production-coordinator regressions cover success, executor failure, first/middle/last check failure, negative executor with passing checks, needs-input, an approval gate after failure, explicit stop, cancellation, timeout and unconfirmed process cleanup. They resolve each returned evidence reference against persisted events and check for one executor invocation and unchanged source. The native adapter propagates cleanup uncertainty to Core's existing fatal boundary instead of returning an ordinary failure. The older handoff regression now keeps the production-generated workflow instead of replacing it with a test graph.
- Actual native Codex **0.153.4**, existing login, no API key: disposable read-only failure run `3d891210-4135-4b38-89a7-33ec843a3b80` reports executor failure and independently records **test failed, build passed, diff passed**. Healthy read-only run `3800159c-21ee-4e02-b936-10d54c39a8f4` reports executor success and **all three checks passed**. Both resolve three persisted evidence references through the same transport-neutral result API used by the browser. Source/tests/configuration stay unchanged, real build output is verified, and disposable Projects/coordinators are cleaned up. Neither invocation replays the user's Project task.
- Both `smoke:native-browser` and `smoke:browser` pass in Chromium **149.0.7827.55**, with a negative executor and failed first check followed by a healthy execution. Each transport dispatches twice and returns exactly two same-conversation results; every message contains all three independently recorded checks and matching evidence IDs/exit codes. Native reload, idle coordinator/cold-worker recovery, no bootstrap replay, scope, cancellation/revocation and all 14 safety cases remain covered. These browser pages/executors are fixtures, distinct from the actual native tests above and the pending real ChatGPT acceptance.
- Both browser fixtures retain 3,000 old turns, 60 seconds idle, **0 idle DOM queries**, and one check after 1,000 mutations. TaskDuration deltas: native **0.0492s**, HTTP **0.0551s**. No idle timer or page polling was added.
- `pnpm lint`, `pnpm format:check`, `pnpm check`, `pnpm test` (**2,101 passed**) and `pnpm build` pass. Source and CLI-packaged extension directories contain the same 14 files byte-for-byte. This update changes local execution composition and evidence handling; the existing browser binding can remain. The personal coordinator idle-stopped normally, so the next new request loads rebuilt local code. Test-owned browser profiles/processes are removed; the user's failed handoff/result/source are preserved. Stop after focused commits/push for real same-conversation re-acceptance, without starting dependent TODOs.
...
A separate regression reproduced a paused/stopped conversation reloading in another tab and replacing the current active binding. Restoration now updates only that paused conversation's routing record; the other tab retains its binding and can dispatch exactly once. This regression also failed before the repair. This establishes a concrete isolation defect, not a unique explanation for every previously missed real turn.

Verification passed: `pnpm lint`, `pnpm format:check`, `pnpm check`, `pnpm test` (**2,173 tests**, including 169 extension tests), `pnpm build`, and both extension browser smokes. HTTP and two consecutive final native smoke runs each executed twice, automatically returned two results and persisted separate pass/fail reviews. Native review restoration, extension reload, cold worker/coordinator recovery, cancellation/Unbind, 14 composer safety cases and mutation-burst deduplication passed. With 3,000 old turns, each 60-second idle measurement recorded zero DOM queries (TaskDuration: HTTP 0.0566s; native 0.0546s and 0.0721s). The real user registry hash is unchanged; production and CLI-packaged extension assets match and contain no temporary tracing code.

Failures encountered during verification are retained in the local logs. The native smoke incorrectly assumed its worker was still running after the idle wait; its precondition now starts only the browser worker, without touching the native coordinator, before the explicit eviction/recovery assertion. The CLI cancellation fixture's fixed 500ms delay could abort before the child wrote its workspace marker; it now waits for the child's actual startup acknowledgement, then verifies cancellation and cleanup. One native attempt completed only one of two expected executions before its 45-second deadline. The test now includes fixture state/reply details on that assertion; the subsequent two runs passed. No unique production cause is claimed for that earlier timeout.

Real account evidence: the development agent personally bound the existing durable test Project in a dedicated real ChatGPT Pro conversation. The new binding message was visibly received, ChatGPT replied “准备就绪”, and the binding stayed enabled. A separate fresh read-only task was attempted once, but its user-message delivery and downstream execution could not be confirmed. As of this inspection, expected Run `c58b7c3b-863b-4ea3-b49f-d7416adc7f46` has no handoff/result/review archive. Earlier Run `8ecf59b4-cde7-480b-984f-e9fa91a589f8` and the fixture's pre-existing source change remain preserved; their success is not attributed to this new attempt.
...
**Finish the user-selected existing Codex task repair.** After the Mac/browser is available, verify the loaded extension build and current-document observation in the exact selected ChatGPT conversation. Establish the desktop's shared native connection and verify target availability before any controlled acceptance run; ordinary desktop restart is not sufficient on the installed version. Reconcile existing archives first, never replay the reported research handoff, and keep the real selected-task loop marked incomplete until native execution, same-conversation delivery and persisted review are proved. The tests, live findings and remaining activation boundary are recorded in [handoff observation repair](HANDOFF-OBSERVATION-REPAIR.md). P0.13–P0.15 remain frozen.
