...
Optional `instructionSources` and `projectInstructionState` distinguish captured project rules, role guidance, workflow/group instructions and legacy absence. `ContextProvenance` describes untrusted content with original `EvidenceReference` event/sequence/attempt identities and mapping selectors. Protocol and SDK export these types, their guards, `ProjectInstruction`, and shared `PROMPT_SAFETY_GUIDANCE`. Agent result/error events may carry `inputEventId`; router source metadata may carry `outputEventId`/`sequence`, and subworkflow starts may carry parent provenance. See [prompt safety](PROMPT-SAFETY.md) for authoritative versus provider-claimed sources and limitations.
...
- `step.retrying.delayMs` optionally records bounded backoff. `BudgetLimits` declares optional token/cost ceilings; `budget.checked` records a hook's before/after phase, allowed flag and optional reason. Actual usage remains in the independent agent result, including absence when unknown.
...
- Agent routed/selected/input/started/completed/failed. Optional `agent.routed` carries a versioned, guarded `AgentRoutingDecision` with ordered attempts, explicit policy and a selected binding or blocking reason; see [provider routing](PROVIDER-ROUTING.md). `agent.selected` records the explicit binding, resolved role, requirements and optional descriptor when metadata or requirements are present. `agent.input` records the resolved redacted `AgentInput` before invocation, with matching run/step/attempt identity and no ephemeral execution controls. Result/error events retain their existing meaning.
