...
New Core runs save `projectInstructions` as an empty array or one redacted root `AGENTS.md` snapshot. Older inputs may omit it; resume does not invent a historical capture. `agent.input` retains labeled instruction sources and `context.provenance`; agent results/errors optionally reference that input event. Router input selections identify the exact source event, subworkflow starts capture parent provenance, and approval contexts retain their evidence origins. These optional fields preserve historical compatibility. See [prompt source semantics](PROMPT-SAFETY.md).
