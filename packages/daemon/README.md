...
Dispatch admits at most four active runs, one per Project, with no unbounded queue. It reserves the latest shared-state revision before execution, archives a redacted handoff, and passes its UUID to Core. Existing run IDs are never overwritten/replayed. A pending handoff blocks new dispatch with `run_busy`; crashed or incomplete execution must be inspected through persisted state before reconciliation. Dispatch uses Core's existing workspace policy and workflow retry/approval rules. A trusted local `ExecutionSetup.timeoutMs` can supply the invocation ceiling; it defaults to 120 seconds and is never an IPC/handoff option. The native CLI composition uses 15 minutes for agent execution, with command checks still defaulting to 120 seconds. Core enforces the stricter step/composition deadline. Paused approval runs remain paused; this API does not bypass a gate.
...
The native composition continues to independent configured verification after ordinary failures. The result keeps the last executor's negative outcome and all failed/missing requested checks, even when the workflow reaches its final node. Only actual `verification.completed` events supply verification statuses/references; executor summaries, command lists and native process logs are not promoted to Verifier evidence. Cancellation and human gates remain authoritative.
