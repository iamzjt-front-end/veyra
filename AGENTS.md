...
Veyra is GUI-first: Chrome Side Panel is the primary daily surface, Local Control Center is the secondary surface, and CLI supplies setup/init/open/doctor infrastructure. The popup is only a launcher/status/Diagnostics shortcut. Preserve `apps/tui` as deferred/optional, with no P0/P1 work. Shared `packages/ui`, Side Panel and the `ve open` Control Center are implemented. The latest explicit user decision prioritizes GUI-1–GUI-6 before real P0.12 re-acceptance. After verified GUI screenshots and focused commits/push, stop for the user’s first visual review; do not automatically start P0.13–P0.15.
